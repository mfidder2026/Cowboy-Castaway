...
#define JUMP_CLIMB    3
#define JUMP_LOOK     4
#define JUMP_CLIMB_DOWN 9  // New state for climbing down (avoid conflict with old states)
#define JUMP_DONE     10 // New state for completion
...
static void jump_tick(void)
{
    ...
    switch (jump_state)
    {
        ...
        case JUMP_CLIMB:
            ...
            if (++jump_timer >= 8)
            {
                ...
                if (++jump_step >= 14 && cow_y <= JUMP_TOP_Y)
                {
                    ...
                    jump_timer = 60;         /* kijken rond (verlengd)      */
                    jump_look_dir = 0;
                }
            }
            break;
        ...
        case JUMP_LOOK:
            ...
            if (--jump_timer == 0)
            {
                jump_state = JUMP_CLIMB_DOWN;  // state 9
                jump_timer = 0;
                jump_step  = 0;
            }
            break;
        ...
        case JUMP_CLIMB_DOWN:  // state 9
            /* Klim in kleine stapjes naar beneden, wisselend tussen up en idle. */
            if (++jump_timer >= 8)
            {
                jump_timer = 0;
                if ((jump_step & 1) == 0)
                {
                    if (cow_y < JUMP_TARGET_Y) cow_y += 2;
                    load_dir(DIR_DOWN);
                    loaded = DIR_DOWN;
                }
                else
                ...
                POKE(SPRITE_PTR, FIRST_BLOCK);
                set_sprite_pos(0, cow_x, cow_y);
                if (++jump_step >= 14 && cow_y >= JUMP_TARGET_Y)
                {
                    jump_state = JUMP_DONE;  // state 10
                    jump_timer = 20;
                    load_idle();
                    loaded = LOADED_IDLE;
                }
...
static unsigned char request_anim(unsigned char n)
{
    ...
    if (animations[n].flags & (ANIM_AT_SHORE | ANIM_TO_LEFT))
    {
        ...
        /* For ANIM_AT_SHORE: walk to water first, then start animation */
        if (animations[n].flags & ANIM_AT_SHORE)
        {
            anim_pending = n;
            return want_left ? DIR_LEFT : DIR_RIGHT;
        }
        /* For ANIM_TO_LEFT (fire animation): start immediately with correct facing */
        else
        {
            anim_start(n);
            return 0xFF;
